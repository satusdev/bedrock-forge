...
export class SecurityService {
	...
	async triggerServerScan(
		...
		const createdScans = await this.prisma.$transaction(
			types.map(scanType =>
	...
	async getOverview() {
		...
		return {
			...
			totals: {
				servers_scanned: serverSummaries.filter(s => s.last_scanned_at !== null)
					.length,
				environments_scanned: environmentSummaries.filter(
					e => e.last_scanned_at !== null,
				).length,
