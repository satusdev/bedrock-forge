...
export class ThemeScanProcessor extends WorkerHost {
  ...
  private async processScan(job: Job) {
    ...
    try {
      ...
      this.logger.error(
        `Theme scan job ${job.id} failed: ${err instanceof Error ? err.message : String(err)}`,
      );
  ...
  private async processManage(job: Job) {
    ...
    try {
      ...
      switch (action) {
        ...
        case "update-all":
          cmd = buildWpCmd(
            `theme update --all --skip-plugins --path=${shellQuote(wpPath)}`,
          );
          break;
        default:
      ...
      this.logger.error(
        `Theme manage job ${job.id} failed: ${err instanceof Error ? err.message : String(err)}`,
      );
