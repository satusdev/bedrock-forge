...
describe('SecurityHardeningService', () => {
	...
	describe('processServerHardening', () => {
		it('successfully runs server hardening actions and updates execution status', async () => {
			...
			(hardeningActions.applyServerHardeningActions as jest.Mock).mockResolvedValue([
				{ action: 'SSH_PORT_CHANGE', status: 'success', detail: 'Port changed to 2222' },
			]);
	...
	describe('processEnvironmentHardening', () => {
		it('successfully runs environment hardening actions and updates execution status', async () => {
			...
			(hardeningActions.applyEnvironmentHardeningActions as jest.Mock).mockResolvedValue([
				{ action: 'DISABLE_FILE_EDIT', status: 'skipped', detail: 'Already disabled' },
			]);
