...
export const pluginsApi = {
	searchWpOrg: (query: string) =>
		api.get<any[]>(`/plugin-scans/search-wp-org?query=${encodeURIComponent(query)}`),

	installPlugin: (
		envId: number,
		body: { slug: string; version?: string; workflow: 'composer' | 'manual'; skipSafetyBackup: boolean },
	) =>
	...
	getJobExecutionLog: (jobExecutionId: number) =>
		api.get<JobExecutionLogStatus>(
			`/job-executions/${jobExecutionId}/log`,
		),

	getCustomCatalog: () =>
		api.get<CustomPlugin[]>('/custom-plugins'),
	...
	removePlugin: (envId: number, slug: string, body: { skipSafetyBackup: boolean }) =>
		api.delete<{ jobExecutionId: number; bullJobId: string }>(
			`/plugin-scans/environment/${envId}/plugins/${slug}`,
			body,
		),

	updatePlugin: (envId: number, slug: string, body: { skipSafetyBackup: boolean }) =>
		api.put<{ jobExecutionId: number; bullJobId: string }>(
			`/plugin-scans/environment/${envId}/plugins/${slug}`,
			body,
		),

	changePluginConstraint: (envId: number, slug: string, body: { constraint: string }) =>
		api.patch<{ jobExecutionId: number; bullJobId: string }>(
			`/plugin-scans/environment/${envId}/plugins/${slug}/constraint`,
			body,
		),

	togglePluginStatus: (envId: number, slug: string, body: { active: boolean; skipSafetyBackup: boolean }) =>
		api.put<{ jobExecutionId: number; bullJobId: string }>(
			`/plugin-scans/environment/${envId}/plugins/${slug}/status`,
			body,
		),

	migrateToComposer: (envId: number, slug: string, body: { skipSafetyBackup: boolean }) =>
		api.post<{ jobExecutionId: number; bullJobId: string }>(
	...
	savePluginUpdateSchedule: (envId: number, body: {
		enabled: boolean;
