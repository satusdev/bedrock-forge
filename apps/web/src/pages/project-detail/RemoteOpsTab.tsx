...
export function RemoteOpsTab({
	...
	const compareQuery = useQuery<{
		rows: { key: string; left: string | null; right: string | null; status: string; is_secret: boolean }[];
	}>({
		...
		enabled: !!selectedEnvId && !!compareRightId && selectedEnvId !== compareRightId,
		retry: false,
	});
	...
	const saveFileMutation = useMutation({
		...
		onSuccess: () => {
			...
			void qc.invalidateQueries({ queryKey: ['remote-file-read', selectedEnvId, openFilePath] });
		},
		onError: err =>
			toast({
	...
	const noteMutation = useMutation({
		...
		onSuccess: () => {
			setNoteBody('');
			void qc.invalidateQueries({ queryKey: ['resource-notes', 'project', projectId] });
		},
	});
	...
	if (!environments.length) {
		return <Card><CardContent className='py-10 text-sm text-muted-foreground'>No environments configured.</CardContent></Card>;
	}

	return (
		<div className='space-y-4'>
			<div className='flex flex-wrap items-center gap-3'>
				<Select value={selectedEnvId ? String(selectedEnvId) : ''} onValueChange={value => setSelectedEnvId(Number(value))}>
				...
				{selectedEnv && <Badge variant='secondary'>{selectedEnv.root_path}</Badge>}
			</div>
			...
			<div className='grid gap-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(340px,.8fr)]'>
				<Card>
					<CardHeader className='flex-row items-start justify-between gap-4'>
						...
						<Button size='sm' variant='outline' onClick={() => void envQuery.refetch()} disabled={envQuery.isFetching}>
					...
					<CardContent className='space-y-3'>
						{envQuery.isLoading ? (
							<div className='text-sm text-muted-foreground'>Loading .env...</div>
						) : envQuery.error ? (
							<div className='rounded-md border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive'>
								{envQuery.error instanceof Error ? envQuery.error.message : 'Unable to load .env'}
							</div>
						) : (
							<>
								{envQuery.data?.missing_required.length ? (
									<div className='flex items-start gap-2 rounded-md border border-amber-300/60 bg-amber-50 p-3 text-sm text-amber-900 dark:bg-amber-950/25 dark:text-amber-200'>
										...
										<span>Missing required keys: {envQuery.data.missing_required.join(', ')}</span>
									</div>
								) : null}
								<Textarea
								...
								<div className='flex justify-end'>
									<Button onClick={() => setConfirmEnvSave(true)} disabled={!envQuery.data || saveEnvMutation.isPending}>
				...
				<div className='space-y-4'>
					<Card>
						...
						<CardContent className='space-y-3'>
							<Select value={compareRightId ? String(compareRightId) : ''} onValueChange={value => setCompareRightId(Number(value))}>
								<SelectTrigger>
									<SelectValue placeholder='Compare with environment' />
								</SelectTrigger>
								<SelectContent>
									{environments.filter(env => env.id !== selectedEnvId).map(env => (
										<SelectItem key={env.id} value={String(env.id)}>{env.type}</SelectItem>
									))}
								</SelectContent>
							</Select>
							<div className='max-h-64 overflow-auto rounded-md border'>
								{compareQuery.data?.rows?.length ? compareQuery.data.rows.map(row => (
									<div key={row.key} className='grid grid-cols-[1fr_auto] gap-3 border-b px-3 py-2 text-xs last:border-b-0'>
										<span className='font-mono'>{row.key}</span>
										<Badge variant={row.status === 'same' ? 'secondary' : 'warning'}>{row.status.replace('_', ' ')}</Badge>
									</div>
								)) : (
									<div className='p-3 text-sm text-muted-foreground'>Select another environment.</div>
								)}
							</div>
						</CardContent>
					</Card>
					...
					<Card>
						...
						<CardContent className='space-y-3'>
							<Textarea value={noteBody} onChange={event => setNoteBody(event.target.value)} placeholder='Add note...' />
							<Button size='sm' onClick={() => noteMutation.mutate()} disabled={!noteBody.trim() || noteMutation.isPending}>
								Add Note
							</Button>
							<div className='space-y-2'>
								{notesQuery.data?.map(note => (
									<div key={note.id} className='rounded-md border bg-muted/30 p-3 text-sm whitespace-pre-wrap'>
										{note.body}
									</div>
								))}
							</div>
				...
				<CardContent className='grid gap-4 lg:grid-cols-[minmax(260px,.8fr)_minmax(0,1.2fr)]'>
					<div className='rounded-md border'>
						<div className='space-y-2 border-b p-2'>
							...
							<Input value={filePath ?? ''} onChange={event => setFilePath(event.target.value)} placeholder='Path' />
							<Button size='icon' variant='outline' onClick={() => void filesQuery.refetch()}>
						...
						<div className='max-h-[440px] overflow-auto'>
							{parentPath && (
								<button className='flex w-full items-center gap-2 border-b px-3 py-2 text-left text-sm hover:bg-muted' onClick={() => setFilePath(parentPath)}>
							...
							{filesQuery.isLoading ? (
								...
								<button
									...
									onClick={() => item.type === 'directory' ? setFilePath(item.path) : setOpenFilePath(item.path)}
								>
									<span className='flex min-w-0 items-center gap-2'>
										{item.type === 'directory' ? <Folder className='h-4 w-4 text-amber-600' /> : <File className='h-4 w-4 text-muted-foreground' />}
										<span className='truncate'>{item.name}</span>
									</span>
									<span className='text-xs text-muted-foreground'>{item.type === 'file' ? `${item.size}b` : ''}</span>
								</button>
								))
							) : (
						...
						<div className='space-y-3'>
							<div className='flex items-center justify-between gap-3'>
								<div className='min-w-0 truncate text-sm font-medium'>{openFilePath ?? 'No file selected'}</div>
								<div className='flex items-center gap-2'>
									<Button
										size='sm'
									...
									<Button size='sm' onClick={() => setConfirmFileSave(true)} disabled={!openFilePath || !fileReadQuery.data || saveFileMutation.isPending}>
										{saveFileMutation.isPending ? <Loader2 className='h-4 w-4 mr-1.5 animate-spin' /> : <Save className='h-4 w-4 mr-1.5' />}
										Save File
									</Button>
								</div>
							</div>
							<Textarea
								...
								value={tailing ? tailQuery.data?.lines.join('\n') ?? fileContent : fileContent}
							...
							{tailing && tailQuery.data?.fetched_at && (
								<p className='text-xs text-muted-foreground'>
									Last tail refresh: {new Date(tailQuery.data.fetched_at).toLocaleTimeString()}
								</p>
							)}
						</div>
					</CardContent>
