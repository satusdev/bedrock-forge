...
export function CleanupScheduleCard({
	...
	const [scheduleForm, setScheduleForm] = useState<CleanupScheduleData>(DEFAULT_SCHEDULE);

	const { data: cleanupSchedule, isLoading: scheduleLoading } = useCleanupScheduleQuery(selectedEnvId);
	...
	const upsertScheduleMutation = useUpsertCleanupScheduleMutation(selectedEnvId);
	const deleteScheduleMutation = useDeleteCleanupScheduleMutation(selectedEnvId, () => {
		setScheduleForm(DEFAULT_SCHEDULE);
	});
