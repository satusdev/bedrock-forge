...
export const toolsApi = {
  getCleanupSchedule: async (
    envId: number,
  ): Promise<CleanupScheduleData | null> => {
    return api.get<CleanupScheduleData | null>(
      `/environments/${envId}/cleanup-schedule`,
    );
  },

  upsertCleanupSchedule: async (
    envId: number,
    form: CleanupScheduleData,
  ): Promise<unknown> => {
  ...
  toggleMaintenanceMode: async (
    ...
    return api.post(
      `/environments/${envId}/wp-actions/maintenance-mode`,
      payload,
    );
  },
  ...
  runCleanup: async (envId: number, dryRun: boolean): Promise<unknown> => {
    return api.post(`/environments/${envId}/wp-actions/cleanup`, {
      dry_run: dryRun,
    });
  },
};
