		...
		<div className='space-y-5'>
			<PageHeader title='Lighthouse'>
				<Button
					variant='outline'
					size='sm'
					onClick={() => queryClient.invalidateQueries({ queryKey: ['lighthouse'] })}
					disabled={isFetching}
				>
					<RefreshCw
			...
			<section className='grid gap-3 md:grid-cols-2 xl:grid-cols-4'>
				{topAudits.map(audit => (
					<div key={`${audit.environment_id}-${audit.strategy}`} className='border rounded-lg p-4'>
						...
						<div className='mt-5 flex items-end gap-3'>
							<p className={`text-4xl font-semibold ${scoreTone(audit.performance_score)}`}>
			...
			<section className='border rounded-lg overflow-hidden'>
				...
				<div className='divide-y'>
					{history.map(audit => (
						<div key={audit.id} className='p-4 grid gap-3 lg:grid-cols-[1fr_360px]'>
							<div className='min-w-0'>
