...
function GenerateDialog({
	...
	const [fromMonth, setFromMonth] = useState(THIS_MONTH);
	const [toYear, setToYear] = useState(THIS_YEAR);
	const [toMonth, setToMonth] = useState(THIS_MONTH);
	const [isSubmitting, setIsSubmitting] = useState(false);

	const periodIsInvalid = toYear * 12 + toMonth < fromYear * 12 + fromMonth;
	const selectedMonths = periodIsInvalid
		? 0
		: (toYear - fromYear) * 12 + (toMonth - fromMonth) + 1;

	function clampTo(fy: number, fm: number, ty: number, tm: number) {
	...
	// Only show projects that have at least one package — packageless projects
	// cannot generate invoices (the backend would skip/error them anyway).
	const projects = (projectsData?.items ?? []).filter(
		p => p.hosting_package !== null || p.support_package !== null,
	);
	const allSelected =
	...
	function resetState() {
		...
		setFromYear(THIS_YEAR);
		setFromMonth(THIS_MONTH);
		setToYear(THIS_YEAR);
		setToMonth(THIS_MONTH);
	}

	const periodLabel = `${MONTHS[fromMonth - 1]} ${fromYear}${
		selectedMonths > 1
			? ` – ${MONTHS[toMonth - 1]} ${toYear} (${selectedMonths} months)`
			: ''
	}`;
	...
	async function handleSubmit(e: FormEvent<HTMLFormElement>) {
		...
		try {
			if (mode === 'bulk') {
				...
				>('/invoices/generate-bulk', { fromYear, fromMonth, toYear, toMonth });
				const created = results.filter(r => r.invoiceNumber).length;
				const skipped = results.filter(r => r.skipped).length;
				toast({
					title: `Created ${created} invoice(s) for ${periodLabel}${
		...
		>
			<DialogContent className='sm:max-w-md'>
				<DialogHeader>
					...
					<DialogDescription>
						Invoices are calculated as monthly price × number of selected
						months.
					</DialogDescription>
				</DialogHeader>
				<form onSubmit={handleSubmit} className='space-y-4'>
					...
					<div className='space-y-3'>
							...
							<div className='flex gap-2'>
								<Select
									...
									onValueChange={v => {
										const m = Number(v);
										setFromMonth(m);
										clampTo(fromYear, m, toYear, toMonth);
									}}
								>
								...
								<Select
									...
									onValueChange={v => {
										const y = Number(v);
										setFromYear(y);
										clampTo(y, fromMonth, toYear, toMonth);
									}}
								>
						...
						{!periodIsInvalid && selectedMonths > 0 && (
							<p className='text-xs text-muted-foreground'>
								{selectedMonths} month{selectedMonths !== 1 ? 's' : ''} selected
							</p>
						)}
					</div>
					...
					<DialogFooter>
						...
						<Button type='submit' disabled={isSubmitting || periodIsInvalid}>
...
export function InvoicesPage() {
	...
	const markSentMutation = useMutation({
		mutationFn: (id: number) => api.put(`/invoices/${id}/mark-sent`, {}),
		onSuccess: () => {
			qc.invalidateQueries({ queryKey: ['invoices'] });
