...
export function SecurityScanProgress() {
	const [activeJobs, setActiveJobs] = useState<Record<string, { progress: number; step?: string }>>({});
	...
	return (
		<div className="space-y-3 mb-6 animate-in fade-in slide-in-from-top-4 duration-500">
			{jobIds.map(id => {
				...
				return (
					<Card key={id} className="border-primary/20 bg-primary/5 overflow-hidden">
								...
								<div className="flex-1 min-w-0">
									...
									{job.step && (
										<p className="text-[10px] text-muted-foreground mt-1.5 uppercase tracking-wider font-medium">
											Current step: <span className="text-foreground">{job.step}</span>
										</p>
									)}
								</div>
							</div>
