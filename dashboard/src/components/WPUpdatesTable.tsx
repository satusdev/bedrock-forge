...
export function WPUpdatesTable() {
  ...
  const fetchUpdates = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/wp/updates');
  ...
  const triggerUpdate = async (update: WPUpdate) => {
    ...
    try {
      const response = await fetch('/api/wp/updates/bulk', {
        method: 'POST',
              ...
              {updates.map((update) => (
                <tr key={update.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`px-2 py-1 text-xs rounded-full ${typeColors[update.update_type] || 'bg-gray-100'}`}>
                      {update.update_type}
                    </span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">{update.package_name}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className="text-gray-500">{update.current_version}</span>
                    <span className="mx-2">→</span>
                    <span className="text-green-600 font-medium">{update.available_version}</span>
                  </td>
