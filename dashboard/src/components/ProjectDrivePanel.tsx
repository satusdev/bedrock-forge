...
import { 
  Cloud, FolderOpen, Image, FileText, Archive, Upload, Link2, 
  ExternalLink, RefreshCw, CheckCircle, XCircle, Loader2, Plus
} from 'lucide-react';
...
const ProjectDrivePanel: React.FC<ProjectDrivePanelProps> = ({
  ...
  const [activeFolder, setActiveFolder] = useState<'assets' | 'docs' | 'backups'>('assets');
  ...
  const loadFolderContents = async () => {
    ...
    try {
      const endpoint = activeFolder === 'assets' 
  ...
  const initializeDriveFolders = async () => {
    ...
    try {
      const res = await fetch(`/api/v1/gdrive/projects/${projectSlug}/initialize`, {
        method: 'POST',
      ...
      <div className="p-4">
        {isLoading ? (
          ...
          <div className="text-center py-8">
            ...
            <button
              onClick={() => openInDrive(folders.find(f => f.type === activeFolder)?.id)}
      ...
      <div className="border-t border-gray-200 px-4 py-3 bg-gray-50">
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="flex items-center text-gray-400">
            ...
            <span className="text-xs font-mono">{gdriveFolderId?.slice(0, 12)}...</span>
          </div>
        </div>
      </div>
    </div>
