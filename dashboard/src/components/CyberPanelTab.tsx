...
import { 
  Globe, Shield, Database, RefreshCw, Plus, Trash2, Server, 
  CheckCircle, XCircle, AlertCircle, Loader2, Settings, HardDrive
} from 'lucide-react';
...
const CyberPanelTab: React.FC<CyberPanelTabProps> = ({ serverId, serverName }) => {
  const [websites, setWebsites] = useState<Website[]>([]);
  const [databases, setDatabases] = useState<DatabaseInfo[]>([]);
  const [serverInfo, setServerInfo] = useState<any>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState<'websites' | 'databases' | 'settings'>('websites');
  ...
  const loadData = async () => {
    ...
    try {
      // Load websites
      const websitesRes = await fetch(`/api/v1/cyberpanel/servers/${serverId}/websites`);
      if (websitesRes.ok) {
      ...
      // Load databases
      const dbRes = await fetch(`/api/v1/cyberpanel/servers/${serverId}/databases`);
      if (dbRes.ok) {
      ...
      // Load server info
      const infoRes = await fetch(`/api/v1/cyberpanel/servers/${serverId}/info`);
      if (infoRes.ok) {
  ...
  const createWebsite = async () => {
    try {
      const res = await fetch(`/api/v1/cyberpanel/servers/${serverId}/websites`, {
        method: 'POST',
  ...
  const issueSSL = async (domain: string) => {
    try {
      await fetch(`/api/v1/cyberpanel/servers/${serverId}/ssl/${domain}`, {
        method: 'POST'
      });
      loadData();
    } catch (err) {
  ...
  const deleteWebsite = async (domain: string) => {
    ...
    try {
      await fetch(`/api/v1/cyberpanel/servers/${serverId}/websites/${domain}`, {
        method: 'DELETE'
      });
      loadData();
    } catch (err) {
      ...
      {serverInfo && (
        <div className="grid grid-cols-4 gap-4">
          <div className="bg-gradient-to-r from-blue-50 to-blue-100 rounded-lg p-4">
            ...
            <div className="text-2xl font-bold text-blue-800">{serverInfo.cpu_usage}%</div>
          </div>
          <div className="bg-gradient-to-r from-green-50 to-green-100 rounded-lg p-4">
            <div className="text-sm text-green-600">Memory</div>
            <div className="text-2xl font-bold text-green-800">{serverInfo.memory_usage}%</div>
          </div>
          <div className="bg-gradient-to-r from-purple-50 to-purple-100 rounded-lg p-4">
            <div className="text-sm text-purple-600">Disk</div>
            <div className="text-2xl font-bold text-purple-800">{serverInfo.disk_usage}%</div>
          </div>
          <div className="bg-gradient-to-r from-orange-50 to-orange-100 rounded-lg p-4">
            <div className="text-sm text-orange-600">Websites</div>
            <div className="text-2xl font-bold text-orange-800">{websites.length}</div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'databases' && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            {databases.map(db => (
              <div key={db.name} className="bg-white border border-gray-200 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
