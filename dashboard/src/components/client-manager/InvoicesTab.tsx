...
import {
  FileText, Plus, Send, Download, CreditCard, Calendar, DollarSign,
  CheckCircle, Clock, AlertTriangle, XCircle, Eye, Trash2, Loader2
} from 'lucide-react';
...
const InvoicesTab: React.FC<InvoicesTabProps> = ({ clientId, clientName, currency = 'USD' }) => {
  ...
  const loadInvoices = async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`/api/v1/clients/${clientId}/invoices`);
      if (res.ok) {
  ...
  const sendInvoice = async (invoiceId: number) => {
    try {
      await fetch(`/api/v1/invoices/${invoiceId}/send`, { method: 'POST' });
      loadInvoices();
    } catch (err) {
  ...
  const recordPayment = async (invoiceId: number) => {
    ...
    try {
      await fetch(`/api/v1/invoices/${invoiceId}/payment`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-white border rounded-lg p-4">
          ...
          <div className="text-xl font-bold text-orange-700">
            {formatCurrency(invoices.filter(i => i.status === 'pending').reduce((s, i) => s + i.balance_due, 0))}
          </div>
        </div>
        <div className="bg-white border rounded-lg p-4">
          ...
          <div className="text-xl font-bold text-red-700">
            {formatCurrency(invoices.filter(i => i.status === 'overdue').reduce((s, i) => s + i.balance_due, 0))}
          </div>
        </div>
      </div>
