...
import { Play, RotateCcw, Clock, CheckCircle, XCircle, AlertTriangle, ArrowRight } from 'lucide-react';
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                        <h2 className="text-lg font-semibold mb-4 text-indigo-600 flex items-center gap-2">
                            <span className="bg-indigo-100 p-1 rounded">1</span> Source: Staging
                        </h2>
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Staging Host</label>
                                <input 
                                    type="text" 
                                    value={stagingHost}
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Staging User</label>
                                <input 
                                    type="text" 
                                    value={stagingUser}
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Staging URL</label>
                                <input 
                                    type="text" 
                                    value={stagingUrl}
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                        <h2 className="text-lg font-semibold mb-4 text-green-600 flex items-center gap-2">
                            <span className="bg-green-100 p-1 rounded">2</span> Target: Production
                        </h2>
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Production Host</label>
                                <input 
                                    type="text" 
                                    value={prodHost}
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Production User</label>
                                <input 
                                    type="text" 
                                    value={prodUser}
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Production URL</label>
                                <input 
                                    type="text" 
                                    value={prodUrl}
                    ...
                    <div className="lg:col-span-2 bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 rounded-xl p-8 text-center border border-gray-200 dark:border-gray-700">
                        <h3 className="text-xl font-bold mb-2 text-gray-900 dark:text-white">Ready to Promote?</h3>
                        <p className="text-gray-500 dark:text-gray-400 mb-6 max-w-2xl mx-auto">
                            This will copy files and database from Staging to Production, run a search-replace for URLs, and flush all caches. This action cannot be easily undone without a backup restore.
                        </p>
                        
                        <div className="flex justify-center items-center gap-4">
                            <div className="text-right">
                                <div className="text-sm font-medium text-gray-500">From</div>
                                <div className="font-mono text-gray-900 dark:text-gray-200">{stagingUrl}</div>
                            </div>
                            <ArrowRight className="text-gray-400" />
                            <div className="text-left">
                                <div className="text-sm font-medium text-gray-500">To</div>
                                <div className="font-mono text-gray-900 dark:text-gray-200">{prodUrl}</div>
                            </div>
                        </div>
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                            {history.length === 0 ? (
                                ...
                                <tr key={deploy.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-300 font-mono">
                                        {deploy.details?.slice(0, 50)}...
                                    </td>
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${
                                            ...
                                            {deploy.status === 'success' ? <CheckCircle className="h-3 w-3" /> : <XCircle className="h-3 w-3" />}
                                            {deploy.status.toUpperCase()}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4">
                                        <button className="text-gray-400 hover:text-indigo-600 transition-colors" title="Rollback to this version">
