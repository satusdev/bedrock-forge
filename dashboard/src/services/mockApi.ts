...
interface MockDashboardApiType {
  ...
  getGoogleDriveAuthStatus: () => Promise<any>
  getDashboardConfig: () => Promise<any>
  executeProjectAction: (projectName: string, action: string, data?: any) => Promise<any>
  updateDashboardConfig: (config: any) => Promise<any>
  updateTheme: (theme: string, primaryColor?: string, accentColor?: string) => Promise<any>
...
export const mockDashboardApi: MockDashboardApiType = {
  ...
  getProjects: () => Promise.resolve({
    data: [
      ...
      { id: 2, name: "Company Site", slug: "company-site", status: "inactive" }
    ]
  }),
