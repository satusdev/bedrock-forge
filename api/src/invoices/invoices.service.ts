...
export class InvoicesService {
	...
	private async validateInvoiceItemLinks(
		clientId: number,
		items: InvoiceCreateDto['items'],
		ownerId?: number,
	) {
		if (!items.length) {
			return;
		...
		if (subscriptionIds.length > 0) {
			...
			for (const item of items) {
				if (
					typeof item.subscription_id !== 'number' ||
				...
				if (
					typeof subscription.project_id === 'number' &&
					subscription.project_id !== item.project_id
				) {
					throw new BadRequestException({
						detail: 'invoice item subscription/project mismatch',
					});
				}
			}
		}
	...
	async getInvoice(invoiceId: number, ownerId?: number) {
		const resolvedOwnerId = this.resolveOptionalOwnerId(ownerId);
	...
	async createInvoice(payload: InvoiceCreateDto, ownerId?: number) {
		const resolvedOwnerId = this.resolveOptionalOwnerId(ownerId);
		const client = await this.prisma.clients.findUnique({
			where: { id: payload.client_id },
			select: { id: true, payment_terms: true, owner_id: true },
		});
		if (!client) {
		...
		await this.validateInvoiceItemLinks(
			payload.client_id,
			payload.items,
			resolvedOwnerId,
		);
	...
	async deleteInvoice(invoiceId: number, ownerId?: number) {
		const invoice = await this.getInvoice(invoiceId, ownerId);
	...
	async sendInvoice(invoiceId: number, ownerId?: number) {
		const invoice = await this.getInvoice(invoiceId, ownerId);
		if (invoice.status !== 'draft') {
			throw new BadRequestException({
	...
	async getInvoicePdfMetadata(invoiceId: number, ownerId?: number) {
		const invoice = await this.getInvoice(invoiceId, ownerId);
	...
	async getInvoiceStats(periodDays = 30, ownerId?: number) {
		const resolvedOwnerId = this.resolveOptionalOwnerId(ownerId);
