...
export class BackupsRunnerService {
	...
	private readonly staleMinutes = Math.max(
		...
		Math.min(
			24 * 60,
			Number.parseInt(process.env.BACKUP_STALE_MINUTES ?? '30', 10) || 30,
		),
	);
	private readonly retentionEnabled =
		(process.env.BACKUP_RETENTION_ENABLED ?? 'true').toLowerCase() !== 'false';
	...
	private readonly fileCleanupDryRun =
		(process.env.BACKUP_FILE_CLEANUP_DRY_RUN ?? 'false').toLowerCase() !==
		'false';
	...
	async runPendingBackups() {
		...
		try {
			...
			if (claims.length > 0) {
				this.logger.log(
					`Claimed ${claims.length} pending backup(s) for execution`,
				);
			}
			...
			this.backupsService.recordPendingRunnerSnapshot({
			...
			if (claimedCount > 0 || failedCount > 0 || loopError) {
				this.logger.log(
					`Pending backup loop summary claimed=${claimedCount} processed=${processedCount} failed=${failedCount} duration_ms=${durationMs}`,
				);
			}
	...
	async runMaintenance() {
		...
		try {
			...
			if (
				...
				this.logger.log(
					`Backup maintenance summary stale=${staleMarked} pruned=${prunedCount} cleanup_deleted=${cleanupDeleted} cleanup_failed=${cleanupFailed} duration_ms=${durationMs}`,
				);
			}
