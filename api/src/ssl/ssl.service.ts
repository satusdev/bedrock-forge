...
export class SslService {
	...
	private async getCertificateRow(certId: number, ownerId?: number) {
		...
		const rows = await this.prisma.$queryRaw<DbSslRow[]>`
			...
			WHERE s.id = ${certId}
				AND (${resolvedOwnerId ?? null}::int IS NULL OR p.owner_id = ${resolvedOwnerId ?? null})
			LIMIT 1
		`;
		const cert = rows[0];
		if (!cert) {
	...
	private async ensureOwnedProject(projectId: number, ownerId?: number) {
		...
		const rows = await this.prisma.$queryRaw<{ id: number }[]>`
			...
			FROM projects
			WHERE id = ${projectId}
				AND (${resolvedOwnerId ?? null}::int IS NULL OR owner_id = ${resolvedOwnerId ?? null})
			LIMIT 1
		`;
		if (!rows[0]) {
	...
	private async ensureDomainLinkIntegrity(
		domainId: number,
		projectId: number,
		ownerId?: number,
	) {
		...
		if (
			typeof domain.project_id === 'number' &&
			domain.project_id !== projectId
		) {
			throw new BadRequestException({
				detail: 'Domain project mismatch',
			});
		}
	}
	...
	async createCertificate(payload: SslCreateDto, ownerId?: number) {
		...
		if (typeof payload.domain_id === 'number') {
			await this.ensureDomainLinkIntegrity(
				payload.domain_id,
				payload.project_id,
				ownerId,
			);
