...
describe('ProjectsService', () => {
	...
	it('normalizes wp_path before creating an environment link', async () => {
		prisma.projects.findUnique.mockResolvedValueOnce({
			id: 1,
			name: 'Acme Site',
			owner_id: 1,
		...
		prisma.projects.update.mockResolvedValueOnce({ id: 1 });
		prisma.monitors.findFirst.mockResolvedValueOnce(null);
		prisma.monitors.create.mockResolvedValueOnce({ id: 301 });

		await service.linkEnvironment(1, {
			environment: 'staging',
			server_id: 2,
			wp_url: 'https://staging.acme.test',
			wp_path: 'var//www/acme/web/',
			database_name: 'acme',
			database_user: 'acme',
			database_password: 'secret',
			is_primary: true,
		...
		expect(prisma.project_servers.create).toHaveBeenCalledWith(
			expect.objectContaining({
				data: expect.objectContaining({
					wp_path: '/var/www/acme',
				}),
	...
	it('normalizes wp_path before updating an environment link', async () => {
		prisma.project_servers.findFirst.mockResolvedValueOnce({
			id: 11,
			project_id: 1,
			server_id: 2,
		...
		await service.updateEnvironment(1, 11, {
			wp_path: 'var//www/acme/web/',
		});

		expect(prisma.project_servers.update).toHaveBeenCalledWith(
			expect.objectContaining({
				data: expect.objectContaining({
					wp_path: '/var/www/acme',
				}),
