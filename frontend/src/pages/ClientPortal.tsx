...
export default function ClientPortal() {
	...
	const [isBootstrappingSession, setIsBootstrappingSession] = useState(
		Boolean(localStorage.getItem('client_token')),
	);
	...
	const { data: projectsData, refetch: refetchProjects } = useQuery({
		...
		enabled: Boolean(activeToken) && !isBootstrappingSession,
	});
	...
	useEffect(() => {
		...
		// Bootstrap once on mount for existing local token.
	...
	const { data: ticketDetailData, refetch: refetchTicketDetail } = useQuery({
		...
		enabled:
			Boolean(activeToken && selectedTicketId) && !isBootstrappingSession,
	});
	...
	const { data: invoiceDetailData, refetch: refetchInvoiceDetail } = useQuery({
		...
		enabled:
			Boolean(activeToken && selectedInvoiceId) && !isBootstrappingSession,
	});
	...
	const handleLogout = async () => {
		try {
			...
			// Ignore backend logout errors; client-side session must still be cleared.
		} finally {
			localStorage.removeItem('client_token');
			setActiveToken('');
			setEmail('');
	...
	return (
		<div className='min-h-screen bg-gray-50 py-8'>
			<div className='max-w-6xl mx-auto px-4 space-y-6'>
				...
				{(!activeToken || isBootstrappingSession) && null}

				{activeToken && !isBootstrappingSession && (
					<>
